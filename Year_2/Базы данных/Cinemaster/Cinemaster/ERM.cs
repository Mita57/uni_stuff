...
namespace Cinemaster
{
    ...
    public class ERM
    {
        ...
        static List<Session> getSessions()
        {
            ...
            String query = String.Format("SELECT films.name, rooms.room, * FROM sessions INNER JOIN films ON sessions.filmID = films.filmID INNER JOIN rooms ON sessions.roomID = rooms.roomID");
            SqlCommand command = new SqlCommand(query, connection);
            SqlDataReader dataReader = command.ExecuteReader();
            while (dataReader.Read())
            {
