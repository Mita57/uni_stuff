     ...
     <div class="twelve columns header_nav">
         <div class="row">
            ...
            <ul id="menu-header" class="nav-bar horizontal">
                <li style="font-size: 18pt; padding: 5px;">ЛАНГУАГЕС</li>
            </ul>
                         ...
                         @if(!in_array($course->id,$checkedCourses))
                             @if($course->number>0)
                                 @if($course->time >\Illuminate\Support\Carbon::now())
                                    ...
                                    @else
                                     <p><strong>Нельзя записаться на курс, так как первое занятие уже прошло</strong></p>
                                    @endif
                                 @else
                                 <p><strong>Нельзя записаться на курс, так как группа набрана полностью</strong></p>
                             @endif
                         @else
                             @if($course->time >\Illuminate\Support\Carbon::now())
