...
class IndexController extends Controller
{
    ...
    function EnrollOnCourse(Request $request, $courseId, $courseName) {
        ...
        $this->validate($request,['town' => 'required|string', 'phone' => 'required',
            'card_number' => 'required','card_time_limit_1' => 'required|min:1|max:12',
            'card_time_limit_2' => 'required|min:0', 'card_code'=>'required']);
        $preData = $request->all();
    ...
    function SortCourses(Request $request){
        ...
        $user = User::getCurrentUserLogin();
        $userId = Auth::id();
        return view('index', compact('user', 'userId','courses','userObj'));
    }
